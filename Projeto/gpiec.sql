...
DELETE FROM mysql.user WHERE user='Administrador' and host='%';
DELETE FROM mysql.user WHERE user='Administrador' and host='localhost';
DELETE FROM mysql.user WHERE user='GpiecViewer' and host='%';
DELETE FROM mysql.user WHERE user='GpiecViewer' and host='localhost';

FLUSH PRIVILEGES;

CREATE USER 'Administrador'@'%' IDENTIFIED BY 'GpiecAdmin';
...
CREATE USER 'GpiecViewer'@'localhost' IDENTIFIED BY 'GpiecViewer';
CREATE USER 'GpiecViewer'@'%' IDENTIFIED BY 'GpiecViewer';

GRANT SELECT ON gpiec.* TO 'gpiecViewer'@'localhost';
GRANT SELECT ON gpiec.* TO 'gpiecViewer'@'%';
FLUSH PRIVILEGES;
...
CREATE TABLE ordemservico (
  ...
  detalhesproblema NVARCHAR(400) NOT NULL  ,
  systemdate TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  data_fechamento VARCHAR(10) NULL,
  comentariosfechamento NVARCHAR(400) NOT NULL,
...
CREATE TABLE feedback (
  ...
  systemdate TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
PRIMARY KEY(idfeedback, fkidusuario),
INDEX feedback_FKIndex1(fkidusuario),
  FOREIGN KEY(fkidusuario)
    REFERENCES usuario(idusuario)
      ON DELETE CASCADE
      ON UPDATE CASCADE);